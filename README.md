...
Documentações em texto podem ser colocadas no diretório `rawdocs/`. Qualquer arquivo listado em `rawdocs/docs.json` aparecerá automaticamente no menu lateral. O conteúdo é traduzido para Português, Alemão ou Inglês de acordo com o idioma selecionado nos botões do topo da página.
...
5. Para disponibilizar textos simples a serem traduzidos automaticamente, coloque arquivos `.txt` em `rawdocs/` e adicione suas informações no arquivo `rawdocs/docs.json`.
